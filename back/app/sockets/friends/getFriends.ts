...
const friends = (instance: Socket, socket: any) => {
  socket.on('GET::FRIENDS', async (userId) => {
    ...
    const friendsStatus: Array<string> = cursor.friends.map(({ status }) => status)
    const requestedBy: Array<string> = cursor.friends.map(({ requestedBy }) => requestedBy)

    let i = 0
    for await (const friend of cursor.friends) {
